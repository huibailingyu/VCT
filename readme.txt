1. 安装：
  ...
  1.2. 安装后有两个文件, vct.exe 和 setting.ini；
  1.3. vct.exe为执行程序，使用Delphi 2010开发，双击运行；
  1.4. setting.ini为配置文件，主要是程序临时文件的输出目录。这个目录在程序启动和退出的时候都会被清空。
       因此请使用一个空目录，不要被它删除了你机器上的有用文件。
  1.5. 需要系统内有ffmpeg ffprobe程序，否则程序提示错误；
  1.6. 只能在windows系统上运行.测试不充分，目前只在三台机器上测过（都是win10系统），所以不确定是否会有些库找不到的情况；
  1.7. 错误处理可能不足，如果出现程序错误，请强制终止程序并重新来过。请多些耐心 :)
...
6. 实现机制
   ...
   6.1.2 各输出方式之间的比较 
         ...
         选择图像方式(jpg, png, bmp)，首帧图片延迟较低，可以较快的显示出首帧图像；
           jpg格式，数据量非常小(4K分辨率，可以到1MB),存储较快，但因为是有损压缩，会有些主管质量损失；
